...
export const AddPill = () => {
    ...
    const { addPill, updatePill, deletePill, pill, isAdding, isUpdating } =
        usePills(Number(id));
    ...
    useEffect(() => {
        if (pill && isEditMode) {
            setName(pill.name);
            setForm(pill.form);
            setAmount(pill.amount);
            setUnit(pill.unit);
            setCurrentStock(pill.currentStock ? String(pill.currentStock) : "");
            setDescription(pill.description || "");
            setFrequency(pill.frequency);
            setTimesPerDay(pill.timesPerDay);
        }
    }, [pill, isEditMode]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditMode) {
            updatePill(
                { id: Number(id), pill: pillData },
                { onSuccess: () => navigate("/") }
            );
        } else {
            addPill(pillData, { onSuccess: () => navigate("/") });
        }
    };

    const handleDelete = () => {
        if (confirm("Are you sure you want to delete this medication?")) {
            deletePill(Number(id), {
                onSuccess: () => {
                    toast.success("Medication deleted"); // <--- Nice feedback
                    navigate("/");
                },
            });
        }
            ...
            <div className="max-w-md mx-auto pb-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-4">
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <input
                                ...
                                className="w-full text-lg border-b-2 border-gray-100 focus:border-blue-500 outline-none py-2"
                                value={name}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <input
                                type="number"
                                placeholder="180"
                                className="w-full font-bold text-lg outline-none text-gray-800"
                                value={currentStock}
                                onChange={(e) =>
                        ...
                        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <div className="flex items-center justify-between gap-1">
                                <div className="flex items-center bg-gray-100 rounded-lg p-1">
                                    ...
                                    <input
                                        type="number"
                                        className="w-8 text-center bg-transparent font-bold text-gray-800 outline-none"
                                        value={amount}
                                        onChange={(e) =>
                                ...
                                <input
                                    type="text"
                                    className="w-full text-right text-sm font-medium text-gray-500 outline-none"
                                    value={unit}
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {(
                                    ...
                                    <button
                                        ...
                                        className={`py-2 px-1 rounded-lg text-xs font-bold border ${
                                            frequency === freq
                    ...
                    <button
                        ...
                        className="w-full bg-gray-900 text-white py-4 rounded-2xl font-bold text-lg shadow-xl hover:bg-black flex items-center justify-center gap-2"
                    >
                        {isAdding || isUpdating ? (
                            "Saving..."
                        ) : (
